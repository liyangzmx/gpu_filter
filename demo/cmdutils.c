...
// 解析参数中是否有 optname 名字的参数, 例如 -loglevel
...
{
    // 解析 -loglevel, 即log级别
    // log级别定义在 <libavutil/log.h>中, 例如: AV_LOG_TRACE
    int idx = locate_option(argc, argv, options, "loglevel");
    const char *env;

    check_options(options);

    // 如果没有解析到 -loglevel, 尝试解析 -v
    if (!idx)
        idx = locate_option(argc, argv, options, "v");
    // 如果解析到了 -v, 按照 "loglevel" 保存设置
    if (idx && argv[idx + 1])
        opt_loglevel(NULL, "loglevel", argv[idx + 1]);
    // 尝试解析report参数, 如果这个参数有, 那么会生成一个类似ffplay-20210527-141801.log的文件
    // -report设置时, log文件中首先打印的是启动时间和写入文件的额路径
    ...
    // 解析是否隐藏banner的参数, 如果设置了 -hide_banner, 打印将减少
...
// 打印程序信息
// INDENT : 是否缩进
// FFMPEG_VERSION : ffmpeg版本, 由Makefile调用 ffbuild/version.sh 
//      生成到 libavutil/ffversion.h 文件当中
...
{
    // 如果ffplay的参数中有: "-version", 则idx不为0
    int idx = locate_option(argc, argv, options, "version");
    // 当设置 -hide_banner 参数或者参数中有 -version 时, 则不打印banner消息
    // 这是因为如果设置了 -version , 则show_version()已经打印过banner了
    if (hide_banner || idx)
        return;

    // 标记 INDENT 与否只影响缩进
    print_program_info (INDENT|SHOW_COPYRIGHT, AV_LOG_INFO);
    // 对于 SHOW_CONFIG , 如果所有的库的编译配置都一样, 则分别打印, 如果不一样, 则
    print_all_libs_info(INDENT|SHOW_CONFIG,  AV_LOG_INFO);
    print_all_libs_info(INDENT|SHOW_VERSION, AV_LOG_INFO);
}

// 与banner的区别是: 不打印各个库的配置, 另外它仅将打印输出到标准输出, 不受 -report参数影响
...
double get_rotation(AVStream *st)
{
    ...
    // 保存元数据中的角度指示
    double theta = 0;
    // 如果成功获取到了展示矩阵, 则获取该值, 并用theta减去该值
    if (displaymatrix)
        theta = -av_display_rotation_get((int32_t*) displaymatrix);

    // 对旋转角度进行弧度的范围限定, 得到(0 ~ 360)之间的数值并保存, 并考虑进位问题
    theta -= 360*floor(theta/360 + 0.9/360);

    // 通过上面的旋转角度判断旋转角度是否是奇数
    if (fabs(theta - 90*round(theta/90)) > 2)
        // 如果角度是奇数, 则打印警告
        //     奇数旋转角度
        //     如果您需要帮助，请将此文件的示例上载到https://streams.videolan.org/upload/，并与ffmpeg-devel邮件列表联系
