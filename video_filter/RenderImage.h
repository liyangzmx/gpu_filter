    ...
    static void allocRenderImage(RenderImage *image) {
        ...
        switch (image->format) {
            ...
            case IMAGE_FORMAT_I420: {
                ...
                break;
            default:
                LogI("RenderImageUtil::allocRenderImage do not support the format. Format = %d",
                     image->format);
                break;
        }
    }
...
//	    LogI("RenderImageUtil::copyRenderImage src[w,h,format]=[%d, %d, %d], dst[w,h,format]=[%d, %d, %d]", src->width, src->height, src->format, dst->width, dst->height, dst->format);
//        LogI("RenderImageUtil::copyRenderImage src[line0,line1,line2]=[%d, %d, %d], dst[line0,line1,line2]=[%d, %d, %d]", src->linesize[0], src->linesize[1], src->linesize[2], dst->linesize[0], dst->linesize[1], dst->linesize[2]);
        ...
        if (src->format != dst->format ||
            ...
            LogI("RenderImageUtil::copyRenderImage invalid params.");
            return;
        }
        ...
        switch (src->format) {
            ...
            case IMAGE_FORMAT_RGBA: {
                ...
                break;
            default: {
                LogI("RenderImageUtil::copyRenderImage do not support the format. Format = %d",
                     src->format);
            }
                break;
        }
    ...
    static void dumpRenderImage(RenderImage *src, const char *fpath, const char *fname) {
        ...
        LogI("dumpRenderImage fp=%p, file=%s", fp, imgPath);
